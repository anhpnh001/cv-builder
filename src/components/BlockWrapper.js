...
function BlockToolbar({ path }) {
  ...
  return (
    <>
      <div className="text-slate-500 p-2 space-x-1 bg-slate-100 rounded-t-xl flex z-10 absolute bottom-full right-0">
        <button
          onClick={handleHideSection}
          className="hover:bg-blue-600 hover:shadow-md hover:text-white w-7 h-7 rounded-lg flex justify-center items-center transition"
        >
          <HiEyeOff size="16" />
        </button>
        <button
...
function BlockWrapper({
  ...
  return (
    <section
      ...
      className="relative hover:bg-slate-100 p-4 transition rounded-l-xl rounded-b-xl"
    >
      {showToolbar && (
        // <div className="bg-gray-100 rounded-l-xl rounded-b-xl absolute -inset-3">
        <BlockToolbar path={path} />
        // </div>
      )}
      <Heading path={path} style={headingStyle} className={headingClassName} />
